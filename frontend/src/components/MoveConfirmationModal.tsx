 ...
 * Purpose: Precision confirmation modal displayed when moving media items
...
export const MoveConfirmationModal: React.FC<MoveConfirmationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-surface-container-low/95 backdrop-blur-md border border-outline-variant/20 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Modal Header */}
        <div className="px-6 pt-6 pb-4 border-b border-outline-variant/15 flex items-start justify-between gap-4">
          ...
          <button
            onClick={onCancel}
            className="p-1.5 text-on-surface-variant hover:text-on-surface rounded-lg hover:bg-white/[0.06] transition-colors cursor-pointer"
            title="Cancel"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-start gap-2.5 p-3.5 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-on-surface leading-relaxed">
          ...
          <div className="space-y-1.5">
            ...
            <div className="max-h-40 overflow-y-auto space-y-1.5 pr-1">
              {conflictedItems.slice(0, 5).map((item) => (
                <div
                  key={item.id}
                  className="flex items-center justify-between p-2.5 bg-surface-container-lowest/80 border border-outline-variant/15 rounded-lg text-xs"
                >
